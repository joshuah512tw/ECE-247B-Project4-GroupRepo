...
class OU_Noise:
    def __init__(
        self,
        action_space: int,
        action_range: list[np.ndarray[float]],
        mu: float = 0.0,
        theta: float = 0.15,
        sigma: float = 0.2,
        seed: int = 42,
    ...
    def _sample(self):
        ...
        # ========== YOUR CODE HERE ==========
        noise = np.random.randn(self.action_space)
    ...
    def noise(self, action: np.ndarray[float]):
        ...
        noised_action = noised_action = np.clip(
            noised_action,
            self.action_range[0],
            self.action_range[1]
        )
...
class DDPG:
    def __init__(
        ...
        self.actor = actor_model(self.observation_space, self.env.action_space.shape[0], **actor_kwargs).to(device)
        self.critic = critic_model(self.observation_space, self.env.action_space.shape[0], **critic_kwargs).to(device)

        self.target_actor = actor_model(self.observation_space, self.env.action_space.shape[0], **actor_kwargs).to(
            device
        )
        self.target_critic = critic_model(self.observation_space, self.env.action_space.shape[0], **critic_kwargs).to(
            device
        )

        self.OU_noise = OU_Noise(
            self.env.action_space.shape[0], [self.env.action_space.low, self.env.action_space.high]
        )
    ...
    def _train_one_batch(self, batch_size):
        ...
        states, actions, rewards, next_states, dones = \
            self.replay_buffer.sample(batch_size, device = self.device)
        ...
        # critic update
        current_q = self.critic(states, actions) # current Q
        critic_loss = self.loss_fn(current_q, y) # critic loss
        
        self.critic_optimizer.zero_grad()
        critic_loss.backward()
        self.critic_optimizer.step() # optimize

        # actor update
        actor_action = self.actor(states)
        actor_loss =  -self.critic(states, actor_action).mean() # actor loss
        
        self.actor_optimizer.zero_grad()
        actor_loss.backward()
        self.actor_optimizer.step() # optimize

        return actor_loss.item(), critic_loss.item() # return losses
    ...
    def train(
        self, episodes: int, val_freq: int, val_episodes: int, test_episodes: int, save_every: int, train_every: int = 1
    ):
        """Train the agent
        ...
        for i in range(episodes):
            ...
            while not done:
                ...
                # train model if conditions are valid
                if len(self.replay_buffer) >= self.batch_size and l % train_every == 0:
                    actor_loss, critic_loss = self._train_one_batch(self.batch_size)
                    actor_loss_total += actor_loss
                    Q_loss_total += critic_loss
            ...
            if self.wandb:
                wandb.log({"total_reward": total_reward, "Q_loss": Q_loss_total, "actor_loss": actor_loss_total})
            print(
                f"Episode {i}: Time: {time.time()-start_time}, Total Reward: {total_reward}, Q Loss: {Q_loss_total}, Actor Loss: {actor_loss_total}"
            )
            ...
            if i % val_freq == val_freq - 1:
                ...
                if self.wandb:
                    wandb.log({"val_mean": val_mean, "val_std": val_std})
        ...
        if self.wandb:
            wandb.log({"test_mean": test_mean, "test_std": test_std})
